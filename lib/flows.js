...
export function runShoppingFlow(session) {
  ...
  // User opens the catalog and chooses a random section with a product suitable for shopping.
  const { section, products } = getRandomSectionWithProducts(
    session,
    (product) => cartProductCandidate(product, initialCart),
  );
...
export function runCheckoutFlow(session) {
  // User opens the catalog and chooses a random section with an available product.
  const { section, products } = getRandomSectionWithProducts(session, checkoutProductCandidate);
