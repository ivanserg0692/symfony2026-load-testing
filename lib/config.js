...
const vus = parsePositiveInt(__ENV.K6_VUS, 1);

if (vus > MAX_LOAD_TEST_USERS) {
  throw new Error(`K6_VUS must be <= ${MAX_LOAD_TEST_USERS} because LoadTestUsersFixture creates ${MAX_LOAD_TEST_USERS} users`);
}
...
export const config = {
  ...
  origin: resolveOrigin(__ENV.BASE_URL),
