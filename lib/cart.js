import { gatewayGetJson, gatewayJsonRequest, gatewayRequest } from './api-client.js';
...
export function getCart(session) {
  return gatewayGetJson('/api/v1/cart', session, 'cart', 'cart', 200, isCart);
}
...
export function deleteCartItem(itemId, session) {
  ...
  return checkHttpResponse(response, `cart delete item ${itemId}`, 204);
}
