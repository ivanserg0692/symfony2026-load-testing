...
export function getSections(session) {
  const body = gatewayGetJson('/api/v1/catalog/sections', session, 'catalog_sections', 'catalog sections', 200, (payload) => (
...
export function getCatalog(sectionId, session) {
  const query = toQuery({
    sectionId,
    active: true,
    page: 1,
  ...
  const body = gatewayGetJson(
    ...
    session,
    'catalog_elements',
    `catalog elements section ${sectionId}`,
    200,
    (payload) => (
      payload !== null
  ...
  return ensureArrayNotEmpty(body.items, `catalog section ${sectionId} products`);
}
