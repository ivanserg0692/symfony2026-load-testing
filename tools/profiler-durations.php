...
function usage(): void
{
    fwrite(STDERR, <<<'TEXT'
Usage:
  php profiler-durations.php [--sort=duration|input] [--format=table|csv] [token...]
...
if (!in_array($sort, ['duration', 'input'], true)) {
    fwrite(STDERR, "Invalid --sort value. Expected duration or input.\n");
    exit(2);
}
...
foreach ($tokens as $position => $token) {
    ...
    $profile = $raw === false ? false : unserialize($raw);
    $time = is_array($profile) ? ($profile['data']['time'] ?? null) : null;

    $duration = is_object($time) && method_exists($time, 'getDuration')
...
if ($format === 'csv') {
    ...
    fputcsv($output, ['token', 'duration_ms', 'method', 'status', 'url', 'path']);
    foreach ($rows as $row) {
        fputcsv($output, [
            $row['token'],
...
printf("%-8s %12s %-6s %-6s %s\n", 'token', 'duration_ms', 'method', 'status', 'url');
foreach ($rows as $row) {
    printf(
        "%-8s %12s %-6s %-6s %s\n",
        $row['token'],
